...
export function validatePublisher(publisher: string): ValidationResult {
  ...
  if (trimmed.length < 2) {
    return {
      isValid: false,
      error: "Publisher name must be at least 2 characters",
    }
  }
...
export function validateAppId(appId: string): ValidationResult {
  ...
  if (!validIdRegex.test(trimmed)) {
    return {
      isValid: false,
      error:
        "App ID can only contain letters, numbers, dots, hyphens, and underscores",
    }
  }
  ...
  if (/^[._-]|[._-]$/.test(trimmed)) {
    return {
      isValid: false,
      error: "App ID cannot start or end with dots, hyphens, or underscores",
    }
  }
...
export function validateImageUrl(url: string): ValidationResult {
  ...
  try {
    ...
    if (!["http:", "https:", "data:"].includes(parsedUrl.protocol)) {
      return {
        isValid: false,
        error: "Image URL must use http://, https://, or data: protocol",
      }
    }
...
export function validateIntentName(intentName: string): ValidationResult {
  ...
  if (trimmed.length < 2) {
    return {
      isValid: false,
      error: "Intent name must be at least 2 characters",
    }
  }
  ...
  if (trimmed.length > 50) {
    return {
      isValid: false,
      error: "Intent name must be less than 50 characters",
    }
  }
  ...
  if (!validIntentRegex.test(trimmed)) {
    return {
      isValid: false,
      error:
        "Intent name can only contain letters, numbers, dots, hyphens, and underscores",
    }
  }
...
export function validateContextType(contextType: string): ValidationResult {
  ...
  if (trimmed.length < 3) {
    return {
      isValid: false,
      error: "Context type must be at least 3 characters",
    }
  }
  ...
  if (trimmed.length > 50) {
    return {
      isValid: false,
      error: "Context type must be less than 50 characters",
    }
  }
  ...
  if (!validContextRegex.test(trimmed)) {
    return {
      isValid: false,
      error:
        "Context type can only contain letters, numbers, dots, hyphens, and underscores",
    }
  }
