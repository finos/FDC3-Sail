        ...
        if (actions){
            ...
            if (actions.intents){
                ...
                actions.intents.forEach((intent : any) => {
                    const msg = {topic:"addIntentListener",source:id, data:{id:guid, intent:intent.intent}};
                    sendMessage(msg);
            ...
            if (actions.contexts){
                ...
                actions.contexts.forEach((context : Context) => {
                    const msg = {topic:"addContextListener",source:id, data:{id:guid, context:context.type}};
                    sendMessage(msg);
...
const wireTopic = (topic : string, config?: any) : void => {
    ...
    (document as any).addEventListener(`FDC3:${topic}`,(e : FDC3Event) => {
        ...
        if (! isVoid){
            const eventId : string | null | undefined = e.detail !== null ? e.detail.eventId : null;
 ...
 export const listen = () => {
    ...
    //set the custom ones...
    wireTopic("joinChannel",{cb:(e : FDC3Event) => { currentChannel = e.detail && e.detail.channel ? e.detail.channel : null ;}});
...
(document as any).addEventListener(TOPICS.CONTEXT_MENU,(event : CustomEvent) => {
...
const actionUrl = async(data : any) : Promise<string> => {
    return new Promise(async (resolve, reject) => {
        ...
        const eventId : string = `getActionUrl_${utils.guid()}`;
        ...
        ipcRenderer.send(TOPICS.FDC3_GET_ACTION_URL,{source:id, eventId:eventId, data:data});
...
ipcRenderer.on(TOPICS.FDC3_CONTEXT,async (event, args) => {
   ...
   if (args.data && args.data.context){
        if (_contextHandlers.indexOf(args.data.context.type) > -1 && directoryData.hasActions){
            ...
            if (ctxUrl && window.location.href !== ctxUrl){
                ...
                document.dispatchEvent(new CustomEvent(TOPICS.NAVIGATE,{
                    detail:{href:ctxUrl}
                }));
                contextSent = true;
            }
        ...
        if (!contextSent) {   
            document.dispatchEvent(new CustomEvent(TOPICS.FDC3_CONTEXT,{
...
ipcRenderer.on(TOPICS.FDC3_INTENT,async (event, args) => {
    ...
    if (!intentSent){
        document.dispatchEvent(new CustomEvent(TOPICS.FDC3_INTENT,{
            detail:{data:args.data, source:args.source}
        })); 
    }
});
