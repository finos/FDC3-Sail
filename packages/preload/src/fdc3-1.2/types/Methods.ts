...
const UnexpectedError = new Error(
  '`fdc3Ready` event fired, but `window.fdc3` not set to DesktopAgent.',
);
  ...
  return new Promise((resolve, reject) => {
    ...
    if (window.fdc3) {
      ...
      const timeout = setTimeout(
        () => (window.fdc3 ? resolve() : reject(TimeoutError)),
        waitForMs,
      );
...
export function raiseIntentForContext(
  context: Context,
  app?: TargetApp,
): Promise<IntentResolution> {
  ...
  if (typeof contextTypeOrHandler !== 'function') {
    return throwIfNoGlobal(() =>
      window.fdc3.addContextListener(
        contextTypeOrHandler as string,
        handler as ContextHandler,
      ),
    );
  } else {
    return throwIfNoGlobal(() =>
      window.fdc3.addContextListener(contextTypeOrHandler as ContextHandler),
    );
  }
}
