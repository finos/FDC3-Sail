...
const dispatchContext = (listener : Listener, listenerId : string, msg : FDC3Message) => {
  ...
  if (!listener.contextType || (listener.contextType && listener.contextType === msg.data.context.type)){
    ...
    const views = getRuntime().getViews();
    const id = listener.appId;
    const view = views.get(id);
    if (view && view.content) {
        view.content.webContents.send(TOPICS.FDC3_CONTEXT,{topic:"context", listenerId:listenerId, data:data, source:msg.source});
    }
...
_listeners.push({
  ...
  handler:(runtime, msg)=>{
    ...
    return new Promise((resolve, reject)=> {
        ...
        const view = msg.source ? runtime.getView(msg.source) : null;
        if (view){
     ...
     // const contextListeners = runtime.getContextListeners();
...
_listeners.push({
    ...
    'handler': (runtime, msg) => {
        ...
        if (targetId){
            console.log(`broadcast message = '${JSON.stringify(msg)}' target = '${targetId}' source = '${msg.source}'`);
            let setPending : boolean = false;
                ...
                target.listeners.forEach(l => {
                    if (!l.intent){
                       if (!l.contextType || (l.contextType && l.contextType === msg.data.context.type)){
                        viewListeners.push({view:target, listenerId:l.listenerId});
                       }        
                    }
                });
                if (viewListeners.length > 0){
                    viewListeners.forEach(viewL => {
                        const data = {"listenerId":viewL.listenerId, "eventId":msg.data.eventId, "ts":msg.data.ts, "context":msg.data.context};
                        viewL.view.content.webContents.send(TOPICS.FDC3_CONTEXT,{topic:"context", listenerId:viewL.listenerId, data:data, source:msg.source});
                    });
                }
                else {
                    setPending = true;
       ...
       const channel = msg.data.channel || source && source.channel || "default";
        ...
        if (channel !== "default"){
            ...
            runtime.getViews().forEach(v => {
                ...
                v.listeners.forEach(l => {
                    const matchChannel = l.channel ? l.channel : v.channel ? v.channel : "default";
  ...
  _listeners.push({
    ...
    'handler':(runtime, msg) => {
      ...
      console.log("fdc3Message recieved",msg);  
        ...
        const result : DirectoryApp = await runtime.fetchFromDirectory(`/apps/${msg.data.name}`);
     ...
     if (result) {
           ...
           if (result && result.start_url){
                ...
                const work = runtime.getWorkspace(msg.source) || sourceView && sourceView.parent;
                const newView = work && work.createView(result.start_url, {directoryData:result});
    ...
    _listeners.push({
      ...
      return new Promise((resolve, reject) => {
          ...
          if (instanceId && view){
             ...
             const target = runtime.getView(instanceId);
             if (target){
                ...
                if (target.pendingContexts && target.pendingContexts.length > 0){
                    target.pendingContexts.forEach((pending, i) => {
                        ...
                        if (pending && pending.source && pending.source === view.id){
                            ...
                            if (pending.context && pending.context.type && pending.context.type === msg.data.type){
                                view.content.webContents.postMessage(TOPICS.FDC3_CONTEXT, {"topic":"context", "data":pending.context, source:source});      
          ...
          const channel = msg.data !== null && msg.data.channel ? msg.data.channel  : "default"; //: (c && c.channel) ? c.channel
          ...
          //const contextListeners = runtime.getContextListeners();
         // console.log("addContextListener - handler", channel, msg);
         ...
         /* contextListeners.get(channel).set(msg.data.id,  {
              ...
              const contexts = getRuntime().getContexts();
              console.log("addContextListener - contexts",  contexts);*/
  ...
  _listeners.push({
      ...
      handler: (runtime, msg) => {
        return new Promise(async (resolve, reject) => {
            ...
            const view = runtime.getViews().get(msg.source);
            if (view){
            ...
            if (pending_intents.length > 0){
                ...
                pending_intents.forEach((pIntent, index) => {
                    ...
                    if (pIntent.viewId === msg.source && pIntent.intent === name){
                        ...
                        if (view && view.content){
                            view.content.webContents.send(TOPICS.FDC3_INTENT,{"topic":"intent", "data":{"intent":pIntent.intent, "context": pIntent.context}, "source":pIntent.source});    
                        }
  ...
  export const joinViewToChannel = (channel : string, view : View, restoreOnly? : boolean) : Promise<void> => {
        ...
        if ( channelContext){
            ...
            if (ctx && !restoreOnly){
                // send to individual listenerIds
                    view.listeners.forEach((l) => {
                        ...
                        if (!l.intent){
                            ...
                            if ((!l.channel || (l.channel && l.channel === channel)) && (!l.contextType || (l.contextType && l.contextType === ctx.type))){
                                view.content.webContents.send(TOPICS.FDC3_CONTEXT,{"topic":"context", "data":{"context": ctx,"listenerId":l.listenerId}, "source":view.id});  
                                contextSent = true;  
                            }
                        }
                    });
                if (!contextSent){
                    ...
                    setPendingContext(view.id, view.id, channelContext && channelContext[0]);
                }
            }
...
_listeners.push({
  ...
  handler:(runtime, msg) => {
    return new Promise(async (resolve, reject) => {
        const view  = runtime.getView(msg.source);
        if (view){
          await joinViewToChannel(msg.data.channel, view, msg.data.restoreOnly);
          resolve(true);
        }
        else {
            resolve(false);
      ...
      handler:(runtime, msg) : Promise<any> => {
        return new Promise(async (resolve, reject) => {
            ...
            if (intent){
                ...
                try {
                const _r = await  getRuntime().fetchFromDirectory(url);
...
const resolveIntent = async (msg : FDC3Message) : Promise<any> => {
    return new Promise(async (resolve, reject) => {
        //find the app to route to
        const sType = msg.selected ? msg.selected.type : undefined;
        const sView = msg.selected && msg.selected.details && msg.selected.details.instanceId ? getRuntime().getView(msg.selected.details.instanceId) : null;
        ...
        if (sType === "window"){
            ...
            if (appId){
                ...
                if (app && app.content){
                    app.content.webContents.send(TOPICS.FDC3_INTENT,{topic:"intent", data:{intent:msg.intent, context: msg.context}, source:source});    
                    //bringing the tab to front conditional on the type of intent
            ...
            const directoryData = msg.selected && msg.selected.details && msg.selected.details.directoryData;
            let start_url = directoryData && directoryData.start_url;
            let appName = directoryData && directoryData.name;
            let pending = true;
           ...
           // console.log("hasActions",msg.selected.details.directoryData);
            ...
            if (directoryData && directoryData.hasActions){
                ...
                const templateR = await fetch(`${directoryUrl}/apps/${appName}/action`,{headers:{ 'Content-Type': 'application/json' },method:"POST",body:JSON.stringify(body)});
                ...
                if (start_url && directoryData){
                    const view = getRuntime().createWorkspace().createView(start_url, {directoryData: directoryData});
                    if (pending ){
                        getRuntime().setPendingIntent(view.id, source, msg.intent, msg.context || undefined);
                    }
                    
                    resolve({result:true, tab:view.id, source:view.id, version:"1.0"});
                }
                else {
    ...
    'handler':(runtime, msg) => {
        return new Promise(async (resolve, reject) => {
          ...
          /*  msg.source = utils.id(port);*/
            ...
            const intentListeners = runtime.getIntentListeners(msg.data.intent, msg.data.target);
               ...
               intentListeners.forEach((listener) => {
                   ...
                   if (listener.viewId !== msg.source){
                        ...
                        if (view && !r.find(item => {
                            ...
                            r.push({'type':"window",'details':{'instanceId':view.id, 'directoryData':view.directoryData}});
                        }
                   }
                });
            }
            ...
            if (r.length > 0){
                if (r.length === 1){
                    ...
                    if (r[0].type === "window" && r[0].details && r[0].details.instanceId){
                        ...
                        if (view){
                            view.content.webContents.send(TOPICS.FDC3_INTENT,{topic:"intent", data:msg.data, source:msg.source});
                            //bringing the tab to front conditional on the type of intent
                        ...
                        if (r[0].details.directoryData.hasActions){
                            ...
                            const templateR = await fetch(`${directoryUrl}/apps/${r[0].details.directoryData.name}/action`,{headers:{ 'Content-Type': 'application/json' },method:"POST",body:JSON.stringify(body)});
                            const action_url = await templateR.text();                     
                            //if we get a valid action url back, set that as the start and don't post pending data
                            if (action_url){
                                start_url = action_url;
                            ...
                            const view = getRuntime().createWorkspace().createView(start_url,{directoryData:r[0].details.directoryData});
                            //view.directoryData = r[0].details.directoryData;
                                //set pending intent for the view..
                                if (pending){
                                    getRuntime().setPendingIntent(view.id, msg.source, msg.data.intent, msg.data.context);
                                }
                                
                                resolve({result:true, source:msg.source, version:"1.2", tab:view.id});
                    ...
                    const getTitle = (app : FDC3App) => {
                        const view = app.details.instanceId ? runtime.getViews().get(app.details.instanceId) : null;
                        const directory = app.details.directoryData ? app.details.directoryData : null;
                        return directory ? directory.title : view && view.content.webContents && view.content.webContents.hostWebContents ? view.content.webContents.hostWebContents.getTitle() : 'Untitled';
                    ...
                    ipcMain.on(eventId, async (event, args) => {
                            let r = await resolveIntent(args);
                            resolve(r);
                    });
                    ...
                    if (sourceView){
                        getRuntime().openResolver({intent:msg.data.intent, context:msg.data.context}, sourceView,r);
                    }
