...
const createListenerItem = (id:string, handler:ContextHandler, contextType?: string) : ListenerItem => {
...
const createChannelObject = (id : string, type : string, displayMetadata : DisplayMetadata) : any => {
    const channel : any = {};
    channel.id = id;
    channel.type = type;
    ...
    channel.getCurrentContext = (contextType? : string) => {
        return wireMethod("getCurrentContext",{channel:channel.id, contextType:contextType});
    };
    ...
    channel.addContextListener = (contextType? : any, handler? : any) => {
        ...
        _contextListeners.set(listenerId,  createListenerItem(listenerId, thisListener,thisContextType));
 ...
 */
const createAppInstance = (id : string, status : instanceStatus) : AppInstance => {
    const instance : any = {};
    ...
    instance.broadcast = (context : Context) => {
        wireMethod("broadcast", {context:context,instanceId:instance.instanceId}, true);
    };

    instance.onStatusChanged = (handler : (newVal : string, oldVal :string) => {}) => {};
...
const wireMethod = (method :string, detail : FDC3EventDetail, config? : any) : Promise <any | null> => {
    ...
    if (config && config.void){      
        ...
        return new Promise((resolve, reject) => {
            ...
            (document as any).addEventListener(`FDC3:return_${eventId}`,(event : FDC3Event)=>{
                    let r = event.detail;
...
const DesktopAgent  = {
    ...
    addContextListener : (contextType:any, handler? : any) => {
        ...
        const thisListener : ContextHandler = handler ? handler : contextType;
        const thisContextType : string = contextType && handler ? contextType : null;
        const listenerId : string = utils.guid();
    ...
    joinChannel : (channel : string) => {
        return new Promise <void>((resolve, reject) => {
            (document as any).addEventListener(TOPICS.CONFIRM_JOIN,(event : FDC3Event) =>{
                resolve();
            }, {once : true});
 ...
 (document as any).addEventListener(TOPICS.FDC3_CONTEXT,(event : FDC3Event) => {
     const listeners = _contextListeners;
     if (event.detail && event.detail.data.listenerId && listeners.has(event.detail.data.listenerId)){
...
const searchTopic = (topc : string) : string => {
    const result : Array<string>= [];
   const items = document.body.children;
...
window.addEventListener('contextmenu', (e : MouseEvent) => {
   ...
   document.dispatchEvent(new CustomEvent(TOPICS.CONTEXT_MENU, {
       detail:detail
   }));
  })
